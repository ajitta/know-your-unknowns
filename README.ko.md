...
관리하는 운영 루프를 제공합니다 — 원문 11개 예시를 모두 커버하는 스킬 11종
(prototypes가 예시 2개를 커버, loop는 원문 대응 없는 오케스트레이터) +
...
**스킬 한눈에 보기** (원문 11개 예시 커버 — prototypes가 2개 커버, loop는 오케스트레이터):
...
파일을 수정하지 않도록 지시되어 있고(도구 목록에서 Edit/Write 제외 — 단 Bash는 읽기 전용
git 조회용으로 남아 있어 샌드박스 수준의 보장은 아님), 조사 테이블 + 개선된 프롬프트
초안을 돌려줍니다.
...
| 검증 환경 | Claude Code 2.1.207, macOS (2026-07-11): `claude plugin validate` 통과, 실제 PostToolUse 파이프라인 경유 확인, 스크립트는 `tests/`로 커버 |
| 플랫폼 | macOS/Linux 검증(CI). Windows 미검증: hook이 `python3`를 호출하는데 기본 Windows PATH에는 보통 없음 — `python3` alias를 만들거나 명령을 조정하세요 |
