...
| 재확인 | 2026-09-06 — 부모 페이지 데모 수(§5)와 2×2 사분면 도형 구조(§2·§9)를 원본 HTML 재페치로 확정 |
...
unknowns / Unknown knowns / Unknown unknowns**.【검증 — 2026-09-06 원본 HTML 확인.
사분면 도형은 `<div class="hero-fig" aria-hidden="true">` 장식 요소이고 각 칸의 내용은
라벨 `<span class="tag">` 하나뿐 — 사분면별 설명 텍스트는 애초에 존재하지 않는다】
...
9개 카테고리(탐색·계획 3 / 코드 리뷰·이해 3 / 디자인 2 / 프로토타이핑 2 /
일러스트·다이어그램 2 / 덱 1 / 리서치·학습 2 / 리포트 2 / 커스텀 편집 인터페이스 3)에
걸쳐 별도의 데모 20개를 제시하고, unknowns 페이지를 "11개 추가 예시의 병렬 전시"로
링크한다.【검증 — 2026-09-06 재페치. 본문 "Twenty self-contained `.html` files an
agent produced instead of a wall of markdown"과 카테고리별 카운트 합
(3+3+2+2+2+1+2+2+3=20) 일치】 즉 unknowns 페이지는 HTML-효과성 테제의
**불확실성 관리 특화 응용판**이다.
...
- 각 데모의 **내부 인터랙션 전체**(예: 슬라이더의 정확한 파라미터, 카드의 전체 문안)
  ...
  (디자인 4종 스타일명, 맺음 문장)은 단일 페치 근거로 오류 가능성이 상대적으로 높다.
- 저자 귀속은 정황 근거의 결합이며 페이지 내 명시가 아니다(§1).
- 2×2 매트릭스의 사분면별 설명 텍스트는 **존재하지 않는다**(장식용 `aria-hidden`
  도형, 라벨만) — 2026-09-06 원본 HTML 확인으로 조사 시점의 "추출되지 않음" 유보를
  종결했다. 같은 재페치에서 부모 페이지 데모 수도 20개로 확정했다(§5).
