...
- ~~**`teach me`는 두 단어짜리 범용 영어 문구**다~~ — **측정됐고 재현되지 않는다.**
  `evals/neg-generic-howto-does-not-fire-teach-me`(2026-09-06 신설): 답이 정해져 있고
  출력 형식까지 지정된 "teach me how to center a div"에서 teach-me는 뜨지 않고 답이 바로
  나왔다. 위험 항목으로 남기지 않는다.
- **`plan`의 크기 게이트는 절반만 확인됐다.** 한 파일짜리 변경에서 계획 문서는 쓰지 않지만
  (기계 판정 통과), 본문에 결정 카드·기각 대안·검증·롤백·가장 약한 부분을 인라인으로
  재현한다. 위반인지 아닌지는 판사 모델 대기.
  근거: [trigger-eval-v0.5.1.md](trigger-eval-v0.5.1.md) "남은 단서".
...
- [trigger-eval-v0.3.0.md](trigger-eval-v0.3.0.md)의 "6/6 정발화, 오발화 0"은 **v0.3.0 시점의 긴
  description을 측정한 값**이다. 0.4.0과 0.5.1에서 description이 두 번 짧아졌으므로 그 수치는 현재
  상태의 측정치가 아니다 — 인용하지 말 것.
- 현재 상태의 측정치는 [trigger-eval-v0.5.1.md](trigger-eval-v0.5.1.md)다:
  한국어 11/11(문구를 뺀 description으로), 영어 11/11, 네거티브 5/5, 행동 3/3,
  기계 그레이더 23/23, 타임아웃 0. **기계 판정 한정** — "올바른 스킬이 떴는가"까지만이고
  `llm` 루브릭 14건은 판사 모델이 없어 미채점이다.
